...
export default function Sidebar({
  ...
  if (isCollapsed) {
    return (
      <div className="w-16 h-screen bg-slate-900 border-r border-slate-800 flex flex-col items-center py-4 justify-between z-30 shrink-0 select-none">
        <div className="flex flex-col items-center gap-4 w-full">
          ...
          <button
            onClick={onNewChat}
            title="New Chat"
            className="w-10 h-10 rounded-xl bg-teal-500/10 border border-teal-500/30 text-teal-400 hover:bg-teal-500/20 flex items-center justify-center transition-all cursor-pointer"
          >
            <Plus size={18} />
          </button>
        </div>
        ...
        <button
          onClick={onLogout}
          title="Sign Out"
          className="p-2.5 rounded-xl text-slate-400 hover:text-red-400 hover:bg-slate-800 transition-all cursor-pointer"
        >
          <LogOut size={18} />
        </button>
      </div>
  ...
  return (
    <aside className="w-64 h-screen bg-slate-900 border-r border-slate-800/80 text-slate-300 flex flex-col justify-between z-30 shrink-0 select-none font-sans">
      {/* ── TOP HEADER & NEW CHAT ── */}
      <div className="p-3.5 space-y-3">
        <div className="flex items-center justify-between px-1">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-sm font-bold text-white tracking-tight leading-none">Xeva Agent</h2>
              <span className="text-[10px] text-teal-400 font-medium">HRMS Assistant</span>
            </div>
          </div>
        ...
        <button
          onClick={onNewChat}
          className="w-full py-2.5 px-3.5 bg-gradient-to-r from-teal-500 to-cyan-500 hover:from-teal-400 hover:to-cyan-400 text-slate-950 font-semibold text-xs rounded-xl shadow-md shadow-teal-500/20 transition-all flex items-center justify-center gap-2 cursor-pointer"
        >
          <Plus size={16} />
          <span>New Chat</span>
        </button>
        ...
        {pinnedSessions.length > 0 && (
          <div>
            <div className="px-2 mb-1.5 flex items-center gap-1.5 text-[10px] font-bold text-slate-400 uppercase tracking-wider">
              <Pin size={11} className="text-teal-400" />
              <span>Pinned</span>
            </div>
      ...
      <div className="p-3 border-t border-slate-800/80 bg-slate-900/50">
        <div className="flex items-center justify-between p-2 rounded-xl bg-slate-800/60 border border-slate-700/50">
          ...
          <button
            onClick={onLogout}
            title="Sign Out"
            className="p-1.5 rounded-lg text-slate-400 hover:text-red-400 hover:bg-slate-700/60 transition-all cursor-pointer shrink-0"
          >
            <LogOut size={15} />
          </button>
        </div>
    ...
    return (
      ...
      >
        <div className="flex items-center gap-2.5 min-w-0 flex-1 pr-2">
          <MessageSquare size={14} className={`shrink-0 ${isActive ? 'text-teal-400' : 'text-slate-500'}`} />
          ...
          {isEditing ? (
            <div className="flex items-center gap-1 w-full" onClick={e => e.stopPropagation()}>
              <input
                ...
                autoFocus
                className="w-full px-1.5 py-0.5 bg-slate-950 border border-teal-500 rounded text-white text-xs focus:outline-none"
              />
              <button onClick={e => handleSaveRename(session.id, e)} className="text-teal-400 hover:text-teal-300">
                <Check size={13} />
              </button>
              <button onClick={handleCancelRename} className="text-slate-400 hover:text-slate-300">
                <X size={13} />
              </button>
            </div>
          ) : (
        ...
        {!isEditing && (
          <div className="relative shrink-0 flex items-center">
            <button
              ...
              className={`p-1 rounded-lg text-slate-400 hover:text-white hover:bg-slate-700/80 transition-all ${
            ...
            {isMenuOpen && (
              <>
                ...
                <div className="absolute right-0 top-6 w-32 bg-slate-800 border border-slate-700/90 rounded-xl shadow-xl py-1 z-50 text-[11px] space-y-0.5">
                  <button
                    onClick={e => {
                      e.stopPropagation()
                    ...
                    className="w-full px-3 py-1.5 text-left text-slate-300 hover:bg-slate-700 flex items-center gap-2"
                  >
